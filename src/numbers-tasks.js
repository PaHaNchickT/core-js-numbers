...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  const upp = x1 * x2 + y1 * y2;
  const aMod = (x1 * x1 + y1 * y1) ** (1 / 2);
  const bMod = (x2 * x2 + y2 * y2) ** (1 / 2);
...
function roundToPowerOfTen(num, pow) {
  ...
  const round = Math.round(temp).toString().length;
  const out = Math.round(temp).toString();
  return +out.padEnd(pow + round, '0');
}
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
/**
...
function toNumber(value, def) {
  ...
  if (Number.isNaN(+value) !== true && typeof +value === 'number') {
    return +value;
  }
  return def;
}
...
function isPowerOfTwo(num) {
  if (num.toString(2).replace(/0/g, '') === '1') {
    return true;
  }
  return false;
}
 ...
 * 12.345, 1   => '12.3'
 */
function toFixed(numb, fractionDigits) {
  return numb.toFixed(fractionDigits);
}
//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
/**
